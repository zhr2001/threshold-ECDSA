...
static sgx_status_t SGX_CDECL sgx_createZeroSecretSharings(void* pms)
{
	...
	//
	sgx_lfence();
	ms_createZeroSecretSharings_t* ms = SGX_CAST(ms_createZeroSecretSharings_t*, pms);
	ms_createZeroSecretSharings_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_createZeroSecretSharings_t), ms, sizeof(ms_createZeroSecretSharings_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	SS* _in_retval;
